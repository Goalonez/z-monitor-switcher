...
3. 如果无法枚举外接显示器，请给当前用户访问 `/dev/i2c-*` 的权限：
...
- **录制快捷键**：在"管理输入源"对话框中，点击快捷键输入框，按下想要的组合键即可（点击输入框外部取消录制）。Wayland 会话会打开系统快捷键授权窗口，请以系统最终显示的组合键为准。
...
- 确保快捷键没有与系统或其他应用冲突
- Linux Wayland 会话需要在系统弹窗中授权快捷键；如果桌面环境不支持 Global Shortcuts Portal，请改用 X11 会话或系统自身的快捷键方案
